...
Euterpe.PluginSlur = function() {
    ...
    Euterpe.extend(Euterpe.Plugin, PluginSlur, {
        process: function(root, scale, extra) {
            ...
            for (var i = 0; i < notes.length; i++) {
                ...
                for (var j = 0; j < slur_cfg.length; j++) {
                    ...
                    if (cfg.type === "begin" || cfg.type === "cont" || cfg.type === "end") {
        ...
        isMultiVoice: function(note) {
            ...
            if (typeof parent === "undefined" || parent.name !== "Euterpe.Column") {
                return false;
            }
        ...
        makeSlur: function(group, up, scale) {
            ...
            return function(root, scale) {
                ...
                for (var i = 1; i < group.length - 1; i++) {
                    while (true) {
                        var pEq = self.getParabolaEq([ firstX, firstY ], [ vertexX, vertexY2 ], [ lastX, lastY ]);
                        var note = group[i];
                        var y = pEq(note.X);
                        if (up && note.Y > y - 10 * scale || !up && note.Y - y < 10 * scale) {
                            vertexY1 -= 10 * m * scale;
        ...
        getParabolaEq: function(first, mid, last) {
            ...
            var A = [ [ Math.pow(first[0], 2), first[0], 1 ], [ Math.pow(mid[0], 2), mid[0], 1 ], [ Math.pow(last[0], 2), last[0], 1 ] ];
